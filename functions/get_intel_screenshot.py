...
#                logger.info(loading_msg.get_attribute('style'))
#                first_hash = hash(driver.page_source)
#                logger.info('first hash : %s' % first_hash)
#                time.sleep(10)
#                second_hash = hash(driver.page_source)
#                logger.info('second hash : %s' % second_hash)
#                if second_hash == first_hash:
#                    time.sleep(5)
#                    third_hash = hash(driver.page_source)
#                    logger.info('third hash : %s' % third_hash)
#                    if second_hash == third_hash:
#                        attachments_dict['color'] = GREEN
#                        logger.info('5초뒤에도 같은화면')
#                   logger.info('style이 none이었다고함')
#                   first_check = True
#                    time.sleep(3)
#                   if loading_msg.get_attribute('style') == 'display: none;':
#                       logger.info('더블체크도 성공했다고함')
#                        second_check = True
#                       break
#                    else:
#                        first_check = False
